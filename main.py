...
class Audio(object):
    ...
    def __init__(self, callback=None, device=None, input_rate=RATE_PROCESS, pa=pyaudio.PyAudio()):
...
class VADAudio(Audio):
    ...
    def __init__(self, aggressiveness=3, device=None, input_rate=None, pa=None):
        super().__init__(device=device, input_rate=input_rate, pa=pa)
...
def main(ARGS):
    ...
    for frame in frames:
        if frame is not None:
            ...
            # Save the possible speech as a wav file
            import datetime
            
            # Save audio to WAV file
            timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
            filename = f'possible_speech_{timestamp}.wav'
